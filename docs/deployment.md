...
因此 S3 不会缩短前半段，首次可下载时间还会增加一次上传；它能改善的是后续交付线路、减轻应用服务器出口流量，并让多个用户或重复下载复用对象。选择到主要客户端网络质量更好的 region/endpoint 才可能显著提速。对于跨区域或高延迟网络，应从实际客户端网络测试 100MB/1GB 下载、拥塞时段丢包与 Range 多连接表现；地理距离和厂商名称都不能替代实测。
